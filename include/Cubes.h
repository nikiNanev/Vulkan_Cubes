...
struct CubeVertex
{
	...
	static std::array<VkVertexInputBindingDescription, 2> getbindingDescription()
	{
		...
		bindingVertex.stride = sizeof(CubeVertex);
	...
	static std::array<VkVertexInputAttributeDescription, 8> getAttributeDescriptions()
	{
		...
		attrDesc[index].offset = offsetof(CubeVertex, pos);
		...
		attrDesc[index].format = VK_FORMAT_R32G32B32_SFLOAT;
		attrDesc[index].offset = offsetof(CubeVertex, color);
		...
		attrDesc[index].offset = offsetof(CubeVertex, texCoord);
		...
		attrDesc[index].location = 7;
		attrDesc[index].binding = 1;
		attrDesc[index].format = VK_FORMAT_R32_UINT;
		attrDesc[index].offset = offsetof(InstanceData, texIndex);
...
struct Cube
{
	std::vector<CubeVertex> getCubeVertices()
	{
